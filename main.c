...
#define SizeOfA 200000
#define SizeOfB 200000
#define SizeOfC 200000

#define numberOfCons 2
#define numberOfProds 3
#define QUEUESIZE 100

#define period 0     // allazw edw
#define firstPeriod 1000000
#define secondPeriod 100000
#define thirdPeriod 10000

#define ALL_Delay 1000000
#define calibration 3000   // 7000 -- 2000 --
...
void *producer (void *q)
{
    ...
    while(t->TasksToExecute > 0 ){
        ...
        D =   abs(end_Timer_Usec) -  abs(in_Timer_Usec) +2;
        printf("ADD TIME  :%ld in (us) \n",D);
...
void print_Final_Message(timer *t){
    ...
    long int  sum_consC=0;
                    ...
                    bubbleSort(DUR, secondTasks/2);
                    bubbleSort(DUR, thirdTasks/2);



                    median_A= (float)DUR[(firstTasks/2)];
                    median_B= (float)DUR[(secondTasks/2)];
                    median_C= (float)DUR[(thirdTasks/2)];
                    ...
                    for(i=1;i<=secondTasks/2;i++){
                        sum_Average_B=sum_Average_B+abs(DUR[i]);

                    }
                    for(i=1;i<=thirdTasks/2;i++){
                        sum_Average_C=sum_Average_C+abs(DUR[i]);
...
**/
                    ...
                    average_B =abs(sum_Average_B)/(float)secondTasks;
                    average_C =abs(sum_Average_C)/(float)thirdTasks;
                    ...
                    for(i=1;i<=secondTasks/2;i++){
                         sum_B = sum_B + (float)abs((DUR[i] - average_B)*(DUR[i] - average_B))/1000;

                    }

                    for(i=1;i<=thirdTasks/2;i++){
                         sum_C = sum_C + (float)abs((DUR[i] - average_C)*(DUR[i] - average_C))/1000;
                    ...
                    variance_B = abs(sum_B) / ((float)firstTasks/2);
                    variance_C = abs(sum_C) / ((float)firstTasks/2);

                    std_deviation_A = (float)sqrt(variance_A);
                    std_deviation_B = (float)sqrt(variance_B);
                    std_deviation_C =(float)sqrt(variance_C);
                    ...
                    printf("NUMBER OF TIMES --  1 HOUR %d\n",3600);
                    printf(" \n");
                    printf("MEAN OF PRODUCER--CONSUMER  WAITING TIME IS %0.6f IN (ms) \n",(float)average_B/1000);
                    printf(" \n");
                    printf("MEDIAN OF PRODUCER--CONSUMER  WAITING TIME IS %0.6f IN (ms) \n",(float)median_B/1000);
                    printf(" \n");
                    printf("STD_DIVIATION OF PRODUCER--CONSUMER WAITING TIME IS %0.10f IN (ms) \n",(float)std_deviation_B);

                    printf(" \n");
                    printf(" \n");
                    printf("NUMBER OF TIMES --  1 HOUR %d\n",3600);
                    printf(" \n");
                    printf("MEAN OF PRODUCER--CONSUMER  WAITING TIME IS %0.6f IN (ms) \n",(float)average_C/1000);
                    printf(" \n");
                    printf("MEDIAN OF PRODUCER--CONSUMER  WAITING TIME IS %0.6f IN (ms) \n",(float)median_C/1000);
                    printf(" \n");
                    printf("STD_DIVIATION OF PRODUCER--CONSUMER WAITING TIME IS %0.10f IN (ms) \n",(float)std_deviation_C);
                    return;
